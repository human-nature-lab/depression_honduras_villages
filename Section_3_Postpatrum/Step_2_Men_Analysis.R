...
# a combined dataset of male respondents for mixed‐effects logistic regression 
...
#
#   4. Retain only male respondents (gender == 1) and merge household IDs.
#
...
# Number of obs: 593, groups:  Household, 585; name, 580; village, 153
...
joined_df_3 <- dataframe_attributes_fr_3 %>%
  ...
  left_join(all_networks_df_health_3, by = c("name" = "Node")) %>%              # Health-related network data for Wave 3
  left_join(all_networks_df_financial_3, by = c("name" = "Node"))               # Financial network data for Wave 3
...
# ---------------------------
# SELECTED VARIABLES & RENAME INTUITIVELY FOR WAVE 4
# ---------------------------
...
model_post_men_not_relative <- glmer(
...
summary(model_post_men_not_relative)

save(model_post_men_not_relative, file = "../Code/Results/model_not_relative_post_men.rda")
...
# ------------------------------------------------------------------------------
coefficients <- summary(model_post_men_not_relative)$coefficients             # Extract coefficient matrix from model summary
...
keep   <- rownames(model.frame(model_post_men_not_relative))
df_use <- final_df[keep, , drop = FALSE]

## -----------------------------------------------------------------
## 2. Re-fit both models on *that identical data object*
model_post_men_not_relative_full <- update(model_post_men_not_relative, data = df_use)

model_post_men_not_relative_null <- glmer(
...
print(anova(model_post_men_not_relative_null, model_post_men_not_relative_full))

lrt <- anova(model_post_men_not_relative_null, model_post_men_not_relative_full)
chi_sq <- lrt$Chisq[2]
df     <- lrt$Df[2]
pval   <- lrt$`Pr(>Chisq)`[2]
...
save(model_post_men_not_relative_null, file = "../Code/Results/model_not_relative_post_men_null.rda")

##— R^2 computation

##— marginal / conditional R² (Nakagawa & Schielzeth, 2013) —─
r.squaredGLMM(model_post_men_not_relative)
#      R2m        R2c
#  [fixed]  [fixed+random]


##— Tjur’s coefficient of discrimination (logistic only) ——––
r2_tjur(model_post_men_not_relative)   # from performance


##— McFadden, Cox & Snell, Nagelkerke, … (all at once) ——––—
r2(model_post_men_not_relative)        # from performance
