...
# of depression among female respondents. Key steps include:
#
...
#
#   4. Filter to females only (gender == 0) and merge household IDs for Waves 1, 3, 4.
#
...
#Number of obs: 30420, groups:  name, 15583; Household, 12566; village, 175
...
final_df$b0600 <- final_df$b0600.x                                   # Scale b0600.x
...
final_df_1 <- final_df_1 %>%
  ...
  dplyr::select(
    ...
    indigenous_status, FI, network_size, network_density, marital_status, avg_shortest_path, Adv_density, f1500r01,
    village, Household, wave,  avg_neighbors_degree
  )
...
model_women_not_relative <- glmer(
...
summary(model_women_not_relative)

save(model_women_not_relative, file = "../Code/Results/model_not_relative_women.rda.rda")
...
# ------------------------------------------------------------------------------
coefficients <- summary(model_women_not_relative)$coefficients             # Extract coefficient matrix from model summary
...
keep   <- rownames(model.frame(model_women_not_relative))
df_use <- final_df[keep, , drop = FALSE]

## -----------------------------------------------------------------
## 2. Re-fit both models on *that identical data object*
model_women_not_relative_full <- update(model_women_not_relative, data = df_use)

model_women_not_relative_null <- glmer(
...
print(anova(model_women_not_relative_null, model_women_not_relative_full))

lrt <- anova(model_women_not_relative_null, model_women_not_relative_full)
chi_sq <- lrt$Chisq[2]
df     <- lrt$Df[2]
pval   <- lrt$`Pr(>Chisq)`[2]
...
save(model_women_not_relative_null, file = "../Code/Results/model_women_not_relative_null.rda")

##— R^2 computation


##— marginal / conditional R² (Nakagawa & Schielzeth, 2013) —─
r.squaredGLMM(model_women_not_relative)
#      R2m        R2c
#  [fixed]  [fixed+random]
...
r2(model_women_not_relative)        # from performance




## Create Table 1 For Women
