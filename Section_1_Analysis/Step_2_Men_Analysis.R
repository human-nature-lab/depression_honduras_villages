...
# ==============================================================================
# This script processes network and individual survey data from Waves 1, 3, and 4
# to prepare a combined dataset for mixed-effects logistic regression analysis
# of depression among male respondents. Key steps include:
#
#   1. Load required packages:
#        • Matrix        — advanced matrix operations
#        • lme4          — mixed-effects model fitting (glmer)
#        • dplyr, tidyr  — data manipulation and reshaping
#        • MuMIn         — Nakagawa & Schielzeth R²
#        • performance   — pseudo-R² variants and discrimination metrics
#
#   2. Wave-specific merging and processing (for Waves 1, 3, 4):
#        a. Merge network attribute tables (dataframe_attributes_fr_*)
#           with full network edges (all_networks_df_*).
#        b. Join to individual respondent data (individuals_wave*_fr),
#           matching by respondent ID.
#        c. Recode f1500r01: 1 if “Younger than 6 months”, else 0.
#        d. Remove unneeded village-level columns (e.g., travel times, clinics).
#        e. Compute household-level depression average (avg_household),
#           excluding the individual’s own score, and count household size.
#        f. Merge village wealth index from CSV (wave-specific to get village information, wealth is not used).
#        g. Harmonize and rename key survey variables from “.x” sources:
#           • age, marital_status, gender, FI
#
#   3. Variable scaling and renaming:
#        • Age (numeric, scaled)
#        • Education      ← b0100.x
#        • Religion       ← b0600.x (factor)
#        • Indigenous     ← indigenous.binary.x
#        • Network metrics (scaled as appropriate):
#            – percentage_depressed (proportion)
#            – friends             (count)
#            – avg_neighbors_degree
#            – intransitivity_values
#            – Adversaries
#            – network_size, network_density, Adv_density
#            – avg_household, number_household
#
#   4. Filter to males only (gender == 1) and merge household IDs for Waves 1, 3, 4.
#
#   5. Combine processed Wave 1, 3, and 4 datasets via rbind, ensure consistent column
#      naming, convert “depressed” to factor, and clean names.
#
...
#
#   7. Extract model outputs:
#        • Coefficients, standard errors, z-values, p-values
#        • Odds ratios & 95% CIs
#        • Likelihood ratio test against null model
#        • Marginal and conditional R² (Nakagawa & Schielzeth)
#        • Tjur’s R² and other pseudo-R² (performance package)
#
#   8. Save model objects and formatted results for downstream reporting.
# ==============================================================================
...
#Number of obs: 19567, groups:  name, 11600; Household, 9024; village, 175
# ------------------------------------------------------------------------------
...
# ---------------------------
# SCALE SELECTED VARIABLES & ASSIGN INTUITIVE NAMES (WAVE 1)
# ---------------------------
# Convert age to numeric and then scale it.
final_df$age <- as.numeric(final_df$age)                                     # Ensure age is numeric
final_df$age <- final_df$age                                          # Scale age
...
# Create a variable for the percentage of depressed neighbors, scaled.
final_df$percentage_depressed <- ifelse(
  total_neigh == 0,                                                         # If there are no neighbors...
  0,                                                                        # assign 0,
  final_df$depressed_w1_neigh / total_neigh                           # otherwise, scale the proportion
)
# Create a variable for the total number of friends (neighbors), scaled.
final_df$friends <- ifelse(
  total_neigh == 0,                                                         # If there are no neighbors...
  0,                                                                        # assign 0,
  final_df$depressed_w1_neigh + final_df$not_depressed_w1_neigh         # otherwise, scale the total count
)

# ---------------------------
# SCALE ADDITIONAL VARIABLES & RENAME INTUITIVELY (WAVE 1)
# ---------------------------
# Scale wealth and rename columns for intuitive reference.
...
# Scale remaining network variables.
final_df$Adversaries <- final_df$Adversaries                           # Scale Adversaries
final_df$network_size <- final_df$network_size                         # Scale network_size
final_df$network_density <- scale(final_df$network_density)                  # Scale network_density
final_df$Adv_density <- scale(final_df$Adv_density)                          # Scale Adv_density
final_df$avg_household <- final_df$avg_household                     # Scale average household depression
final_df$number_household <- final_df$number_household                # Scale household size
...
final_df$intransitivity_values <- final_df$intransitivity_values     # Scale intransitivity_values
final_df$d0700 <- final_df$d0700.x                                  # Scale d0700.x
final_df$b0100 <- final_df$b0100.x                                   # Scale b0100.x
final_df$b0600 <- as.factor(final_df$b0600.x)                                   # Scale b0600.x
final_df$indigenous.binary <- final_df$indigenous.binary.x           # Scale indigenous.binary.x
...
final_df$percentage_depressed <- ifelse(
  total_neigh == 0,
  0,
  final_df$depressed_w3_neigh / total_neigh         # Scaled proportion of depressed neighbors
)
final_df$friends <- ifelse(
  total_neigh == 0,
  0,
  final_df$depressed_w3_neigh + final_df$not_depressed_w3_neigh  # Scaled total neighbors
)

# Scale additional network-level variables.
final_df$Adversaries <- final_df$Adversaries       # Scale adversaries variable
final_df$network_size <- final_df$network_size     # Scale network size
final_df$network_density <- scale(final_df$network_density)  # Scale network density
final_df$Adv_density <- scale(final_df$Adv_density)         # Scale adversarial density
final_df$avg_household <- final_df$avg_household     # Scale average household depression
final_df$number_household <- final_df$number_household  # Scale household size
...
joined_df_4 <- dataframe_attributes_fr_4 %>%
  ...
  left_join(all_networks_df_health_4, by = c("name" = "Node")) %>%             # Health-related network data for Wave 4
  left_join(all_networks_df_financial_4, by = c("name" = "Node"))              # Financial network data for Wave 4
...
final_df$indigenous.binary <- final_df$indigenous.binary.x            # Scale indigenous.binary.x
final_df$age <- as.numeric(final_df$age)                                     # Convert age to numeric
final_df$age <- final_df$age                                          # Scale age (again, if necessary)
final_df$intransitivity_values <- final_df$intransitivity_values      # Scale intransitivity values
...
final_df$percentage_depressed <- ifelse(                                  # Compute scaled percentage of depressed neighbors
  total_neigh == 0,
  0,
  final_df$depressed_w4_neigh / total_neigh
)
final_df$friends <- ifelse(                                               # Compute scaled total number of friends
  total_neigh == 0,
  0,
...
# Filter to keep only male respondents.
final_df <- final_df %>% filter(gender == 1)                     # Retain only rows where gender == 1
...
summary(model_men_not_relative)

save(model_men_not_relative, file = "../Code/Results/model_not_relative_men.rda")
...
# ------------------------------------------------------------------------------
coefficients <- summary(model_men_not_relative)$coefficients             # Extract coefficient matrix from model summary
...
keep   <- rownames(model.frame(model_men_not_relative))
df_use <- final_df[keep, , drop = FALSE]

## -----------------------------------------------------------------
## 2. Re-fit both models on *that identical data object*
model_men_not_relative_full <- update(model_men_not_relative, data = df_use)

model_men_not_relative_null <- glmer(
...
print(anova(model_men_not_relative_null, model_men_not_relative_full))

lrt <- anova(model_men_not_relative_null, model_men_not_relative_full)
chi_sq <- lrt$Chisq[2]
df     <- lrt$Df[2]
pval   <- lrt$`Pr(>Chisq)`[2]
...
save(model_men_not_relative_null, file = "../Code/Results/model_men_not_relative_null.rda")

##— R^2 computation


##— marginal / conditional R² (Nakagawa & Schielzeth, 2013) —─
r.squaredGLMM(model_men_not_relative)
#      R2m        R2c
#  [fixed]  [fixed+random]


##— Tjur’s coefficient of discrimination (logistic only) ——––
r2_tjur(model_women_not_relative)   # from performance


##— McFadden, Cox & Snell, Nagelkerke, … (all at once) ——––—
r2(model_men_not_relative)        # from performance


## Create Table 1 For Men
...
mean(c(df_sub_1$age, df_sub_3$age, df_sub_4$age))
sd(c(df_sub_1$age, df_sub_3$age, df_sub_4$age))

## Marital_Status

sum(c(df_sub_1$marital_status, df_sub_3$marital_status, df_sub_4$marital_status))

sum(c(df_sub_1$depressed, df_sub_3$depressed, df_sub_4$depressed))

## Education

mean(c(df_sub_1$education, df_sub_3$education, df_sub_4$education))
sd(c(df_sub_1$education, df_sub_3$education, df_sub_4$education))

## Religion

sum(c(df_sub_1$religion, df_sub_3$religion, df_sub_4$religion)==1)
sum(c(df_sub_1$religion, df_sub_3$religion, df_sub_4$religion)==2)


## Indigenous Status

sum(c(df_sub_1$indigenous_status, df_sub_3$indigenous_status, df_sub_4$indigenous_status)==0)
sum(c(df_sub_1$indigenous_status, df_sub_3$indigenous_status, df_sub_4$indigenous_status)==1)

## Food Insecurity

sum(c(df_sub_1$FI, df_sub_3$FI, df_sub_4$FI)==0)
sum(c(df_sub_1$FI, df_sub_3$FI, df_sub_4$FI)==1)
