...
# ==============================================================================
# This script processes data from Waves 1 and 3 for a mixed-effects logistic 
...
#      - Variables are scaled and renamed to:
...
#
#   6. Filtering to retain only female respondents (where gender is assumed to be 0 or 1 as specified).
#
#   7. Merging household information (Household1, Household3, Household4) from the corresponding
#      individuals data.
#
#   8. Combining the processed data from Waves 1 and 3 into one final dataset.
#
...
# OVERALL 10376 WOMEN x 2 waves, that completed both the questioner and the census in 2 waves (wave 1 and 3)
...
# ------------------------------------------------------------------------------
# OVERVIEW:
# This code processes data from Waves 1 and 3. For each wave we:
...
# ---------------------------
# Convert age to numeric and then leave unchanged.
...
final_df$b0600 <- as.factor(final_df$b0600.x)                                  # b0600.x remains unchanged
...
# Combine the datasets from Waves 1 and 3.
...
model_women_not_relative_appendix  <- glmer(
  depressed ~ as.numeric(age) + marital_status +  percentage_depressed +
    ...
    wave + (1 + wave | name) + (1 | Household) + (1 | village),                     # Random intercepts for name, Household, and village
  data = filtered_data,
...
summary(model_women_not_relative_appendix)

save(model_women_not_relative_appendix, file = "../Code/Results/model_not_relative_women_appendix.rda")
...
# ------------------------------------------------------------------------------
coefficients <- summary(model_women_not_relative_appendix)$coefficients             # Extract coefficient matrix from model summary
...
keep   <- rownames(model.frame(model_women_not_relative_appendix))
df_use <- droplevels(filtered_data[keep, , drop = FALSE])   # <─ KEY!


## -----------------------------------------------------------------
## 2. Re-fit both models on *that identical data object*
model_women_not_relative_appendix_full <- update(model_women_not_relative_appendix, data = df_use)

model_women_not_relative_appendix_null <- glmer(
...
print(anova(model_women_not_relative_appendix_null, model_women_not_relative_appendix_full))

lrt <- anova(model_women_not_relative_appendix_null, model_women_not_relative_appendix_full)
chi_sq <- lrt$Chisq[2]
df     <- lrt$Df[2]
pval   <- lrt$`Pr(>Chisq)`[2]
...
save(model_women_not_relative_appendix_null, file = "../Code/Results/model_not_relative_women_appendix_null.rda")


##— R^2 computation

##— marginal / conditional R² (Nakagawa & Schielzeth, 2013) —─
r.squaredGLMM(model_women_not_relative_appendix)
#      R2m        R2c
#  [fixed]  [fixed+random]


##— Tjur’s coefficient of discrimination (logistic only) ——––
r2_tjur(model_women_not_relative_appendix)   # from performance


##— McFadden, Cox & Snell, Nagelkerke, … (all at once) ——––—
r2(model_women_not_relative_appendix)        # from performance
